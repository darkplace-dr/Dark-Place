---@class SharpSliceSpell : Object
---@overload fun(...) : SharpSliceSpell
local SharpSliceSpell, super = Class(Object)

function SharpSliceSpell:init(user)
    super.init(self, 0, 0)
    ...
    self.bg = Assets.getTexture("effects/cardspell/gradient")

    self.bg_snowfall = Assets.getTexture("effects/cardspell/cardfall")
    ...
    Assets.playSound("sharpslice", 0.5)

    self.hurt_enemies = false
end

function SharpSliceSpell:update()
    super.update(self)
...
function SharpSliceSpell:drawTiled(x, y, alpha)
...
function SharpSliceSpell:createSnowflake(x, y)
    local snowflake = SharpSliceSpade(x, y)
