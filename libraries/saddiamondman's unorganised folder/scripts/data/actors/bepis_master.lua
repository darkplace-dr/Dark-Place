local actor, super = Class(Actor, "bepis_master")


function actor:init(chara)
	super.init(self)
	self.name = "bepis_master"
	self.width = 34
	self.height = 34
...
function actor:onSpriteUpdate(sprite)
	if sprite.active == 1 then
		if sprite.rightside == 1 then
			sprite.rightside = 0
			Game.world.timer:tween(sprite.speed * 2, sprite, { bean = sprite.left_bop }, "out-quad",
				function() actor:bopLEFT(sprite) end)
		elseif sprite.leftside == 1 then
			sprite.leftside = 0
			Game.world.timer:tween(sprite.speed * 2, sprite, { bean = sprite.right_bop }, "out-quad",
				function() actor:bopRIGHT(sprite) end)
		end
		if sprite.upside == 1 then
			sprite.upside = 0
			Game.world.timer:tween(sprite.speed, sprite, { stween = sprite.low_bop }, "out-quad",
				function() actor:bopDOWN(sprite) end)
			sprite.beret = 1
		elseif sprite.downside == 1 then
			sprite.downside = 0
			Game.world.timer:tween(sprite.speed, sprite, { stween = sprite.top_bop }, "out-quad",
				function() actor:bopUP(sprite) end)
			sprite.beret = 1
		end
	...
	elseif sprite.active == 2 then
		if sprite.rightside == 1 then
			sprite.rightside = 0
			Game.world.timer:tween(0.1, sprite, { bean = sprite.left_bop * 1.5 }, "out-quad",
				function() actor:bopLEFT(sprite) end)
		elseif sprite.leftside == 1 then
			sprite.leftside = 0
			Game.world.timer:tween(0.1, sprite, { bean = sprite.right_bop * 1.5 }, "out-quad",
				function() actor:bopRIGHT(sprite) end)
		end
