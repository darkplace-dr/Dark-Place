...
function Soul2:onRemove(parent)
    ...
    if parent == Game.battle and Game.battle.soul2 == self then
        Game.battle.soul2 = nil
    end
end

function Soul2:onWaveStart() end

function Soul2:shatter(count)
...
function Soul2:transitionTo(x, y, should_destroy)
...
function Soul2:isMoving()
    return self.moving_x ~= 0 or self.moving_y ~= 0
end

function Soul2:getExactPosition(x, y)
    return self.x + self.partial_x, self.y + self.partial_y
end

function Soul2:setExactPosition(x, y)
    self.x = math.floor(x)
...
function Soul2:moveX(amount, move_y)
...
function Soul2:moveY(amount, move_x)
...
function Soul2:moveXExact(amount, move_y)
...
function Soul2:moveYExact(amount, move_x)
    local sign = Utils.sign(amount)
    for i = sign, amount, sign do
        local last_x = self.x
        local last_y = self.y
...
function Soul2:onDamage(bullet, amount)
    -- Can be overridden, called when the soul actually takes damage from a bullet
end

function Soul2:onCollide(bullet)
    -- Handles damage
    bullet:onCollide(self)
end

function Soul2:onSquished(solid)
