local character, super = Class(PartyMember, "synthlave")

function character:init()
    super.init(self)

    local synthlave_style = Game:getConfig("synthlaveStyle")

    -- Display name
    self.name = "Synthlave"

    -- Actor (handles sprites)
    self:setActor("synthlave")
    ...
    if synthlave_style == 1 then
        self.menu_icon = "party/synthlave/head_ch1"
    else
        self.menu_icon = "party/synthlave/head"
    end
    -- Path to head icons used in battle
    self.head_icons = "party/synthlave/icon"
    -- Name sprite (optional)
    self.name_sprite = "party/synthlave/name"
...
function character:drawPowerStat(index, x, y, menu)
    if index == 1 then
        if Game.chapter == 1 then
            -- Chapter 1 Synthlave "Kindness" stat (doggable)
            if not menu.synthlave_dog then
            ...
            -- Chapter 2 Synthlave "Sweetness" stat (non-doggable)
