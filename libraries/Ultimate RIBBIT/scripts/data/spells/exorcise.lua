local spell, super = Class(Spell, "exorcise")
...
function spell:init()
    ...
    self.cast_name = "EXORCISE"

    -- Battle description
    self.effect = "Begone,\nFreakazoid"
    -- Menu description
    self.description = "Banish wicked spirits with this powerful spell.\nMay work on alive beings too. That sounds fun!"
...
function spell:getCastMessage(user, target)
    ...
    elseif target.mercy < 100 then
        return message.."\n[wait:0.25s]* But the enemy wasn't [color:blue]READY[color:reset]..."
    else
...
function spell:onCast(user, target)
    if target.tired then
        ...
        Game.battle.timer:during(8/30, function()
            recolor.color = MathUtils.lerp(recolor.color, {0, 0, 1}, 0.12 * DTMULT)
        end, function()
            Game.battle.timer:during(8/30, function()
