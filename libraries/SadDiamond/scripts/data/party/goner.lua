local character, super = Class(PartyMember, "goner")

function character:init()
    super.init(self)
    gonername = (Game.save_name)
    -- Display name
    self.name = gonername

    -- Actor (handles overworld/battle sprites)
    self:setActor("goner")
    self:setLightActor("goner")
    ...
    if Game:getFlag("head_0") then
        self.menu_icon = "party/goner/heads/0_head/icon"
    end

    if Game:getFlag("head_1") then
        self.menu_icon = "party/goner/heads/1_head/icon"
    end
...
-- this is meant for kris and not the goner
