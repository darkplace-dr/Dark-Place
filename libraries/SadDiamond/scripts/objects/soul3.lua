local Soul3, super = Class(Object)

function Soul3:init(x, y, color)
    super.init(self, x, y)
    ...
    self.sprite = Sprite("player/heart3_dodge")
...
function Soul3:onRemove(parent)
    super.onRemove(self, parent)

    if parent == Game.battle and Game.battle.soul3 == self then
        Game.battle.soul3 = nil
    end
end

function Soul3:onWaveStart() end

function Soul3:shatter(count)
...
function Soul3:transitionTo(x, y, should_destroy)
...
function Soul3:isMoving()
    return self.moving_x ~= 0 or self.moving_y ~= 0
end

function Soul3:getExactPosition(x, y)
    return self.x + self.partial_x, self.y + self.partial_y
end

function Soul3:setExactPosition(x, y)
    self.x = math.floor(x)
...
function Soul3:moveX(amount, move_y)
...
function Soul3:moveY(amount, move_x)
...
function Soul3:moveXExact(amount, move_y)
...
function Soul3:moveYExact(amount, move_x)
    local sign = Utils.sign(amount)
    for i = sign, amount, sign do
        local last_x = self.x
        local last_y = self.y
...
function Soul3:onDamage(bullet, amount)
    -- Can be overridden, called when the soul actually takes damage from a bullet
end

function Soul3:onCollide(bullet)
    -- Handles damage
    bullet:onCollide(self)
end

function Soul3:onSquished(solid)
    -- Called when the soul is squished by a solid
    solid:onSquished(self)
end

function Soul3:doMovement()
    local speed = self.speed

    -- Do speed calculations here if required.

    if self.allow_focus then
        if Input.down("r") then speed = speed / 2 end -- Focus mode.
    end

    local move_x, move_y = 0, 0

    -- Keyboard input:
    if Input.down("f")  then move_x = move_x - 1 end
    if Input.down("h") then move_x = move_x + 1 end
    if Input.down("t")    then move_y = move_y - 1 end
    if Input.down("g")  then move_y = move_y + 1 end
