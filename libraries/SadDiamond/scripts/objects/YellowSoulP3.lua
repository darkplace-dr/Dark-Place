local YellowSoulP3, super = Class(Soul3)

function YellowSoulP3:init(x, y, angle)
    super:init(self, x, y)
...
function YellowSoulP3:update()
    ...
    if Input.pressed("r") and self.hold_timer == 0 and self:canUseShots() then -- fire normal shot
        self:fireShot(false)
    end
    if self:canUseBigShot() then
        -- check release before checking hold, since if held is false it sets the timer to 0
        if Input.released("r") then -- fire big shot
            ...
            elseif self.hold_timer >= 40 then -- fire big shot
                if self:canCheat() and Input.down("r") then -- they are cheating
                    self:onCheat()
                end
                self:fireShot(true)
        ...
        if Input.down("r") then -- charge a big shot
...
function YellowSoulP3:onRemoveFromStage(stage)
...
function YellowSoulP3:getChargeSpeed()
...
function YellowSoulP3:canUseBigShot() return self.can_use_bigshot end
function YellowSoulP3:canUseShots() return self.can_use_shots end
function YellowSoulP3:canShoot() return self.can_shoot end
function YellowSoulP3:canCheat() return self.allow_cheat end
function YellowSoulP3:isTeaching() return self.teaching end

function YellowSoulP3:fireShot(big)
    local shot
    if big then
