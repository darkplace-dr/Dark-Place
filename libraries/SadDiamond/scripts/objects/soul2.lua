...
function Soul2:doMovement()
    ...
    if self.allow_focus then
        if Input.down("q") then speed = speed / 2 end -- Focus mode.
    end

    local move_x, move_y = 0, 0

    -- Keyboard input:
    if Input.down("a")  then move_x = move_x - 1 end
    if Input.down("d") then move_x = move_x + 1 end
    if Input.down("w")    then move_y = move_y - 1 end
    if Input.down("s")  then move_y = move_y + 1 end
