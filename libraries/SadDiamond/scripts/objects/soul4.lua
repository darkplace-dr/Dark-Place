local Soul4, super = Class(Object)

function Soul4:init(x, y, color)
    super.init(self, x, y)
    ...
    self.sprite = Sprite("player/heart4_dodge")
...
function Soul4:onRemove(parent)
    super.onRemove(self, parent)

    if parent == Game.battle and Game.battle.soul4 == self then
        Game.battle.soul4 = nil
    end
end

function Soul4:onWaveStart() end

function Soul4:shatter(count)
...
function Soul4:transitionTo(x, y, should_destroy)
...
function Soul4:isMoving()
    return self.moving_x ~= 0 or self.moving_y ~= 0
end

function Soul4:getExactPosition(x, y)
    return self.x + self.partial_x, self.y + self.partial_y
end

function Soul4:setExactPosition(x, y)
    self.x = math.floor(x)
...
function Soul4:moveX(amount, move_y)
...
function Soul4:moveY(amount, move_x)
...
function Soul4:moveXExact(amount, move_y)
...
function Soul4:moveYExact(amount, move_x)
    local sign = Utils.sign(amount)
    for i = sign, amount, sign do
        local last_x = self.x
        local last_y = self.y
...
function Soul4:onDamage(bullet, amount)
    -- Can be overridden, called when the soul actually takes damage from a bullet
end

function Soul4:onCollide(bullet)
    -- Handles damage
    bullet:onCollide(self)
end

function Soul4:onSquished(solid)
    -- Called when the soul is squished by a solid
    solid:onSquished(self)
end

function Soul4:doMovement()
    local speed = self.speed

    -- Do speed calculations here if required.

    if self.allow_focus then
        if Input.down("u") then speed = speed / 2 end -- Focus mode.
    end

    local move_x, move_y = 0, 0

    -- Keyboard input:
    if Input.down("j")  then move_x = move_x - 1 end
    if Input.down("l") then move_x = move_x + 1 end
    if Input.down("i")    then move_y = move_y - 1 end
    if Input.down("k")  then move_y = move_y + 1 end
