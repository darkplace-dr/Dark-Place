local Player_3, super = Class(Character)

function Player_3:init(chara, x, y, other)
    super.init(self, chara, x, y, other)

    self.is_player_3 = true
...
function Player_3:getDebugInfo()
...
function Player_3:getDebugOptions(context)
    context = super.getDebugOptions(self, context)
    context:addMenuItem("Toggle force run", "Toggle if the player_3 is forced to run or not", function() self.force_run = not self.force_run end)
    context:addMenuItem("Toggle force walk", "Toggle if the player_3 is forced to walk or not", function() self.force_walk = not self.force_walk end)
    return context
end

function Player_3:onAdd(parent)
    super.onAdd(self, parent)

    if parent:includes(World) and not parent.player_3 then
        parent.player_3 = self
    end
end

function Player_3:onRemove(parent)
    super.onRemove(self, parent)

    self.slide_sound:stop()
    if parent:includes(World) and parent.player_3 == self then
        parent.player_3 = nil
    end
end

function Player_3:onRemoveFromStage(stage)
    super.onRemoveFromStage(stage)
    self.slide_sound:stop()
end

function Player_3:setActor(actor)
...
function Player_3:setState(state, ...)
    self.state_manager:setState(state, ...)
end

function Player_3:isCameraAttachable()
    return not (self.state == "SLIDE" and self.slide_in_place)
end

function Player_3:isCamera2Attachable()
    return not (self.state == "SLIDE" and self.slide_in_place)
end

function Player_3:isMovementEnabled()
...
function Player_3:handleMovement()
    local walk_x = 0
    local walk_y = 0

    if Input.down("h") then walk_x = walk_x + 1 end
    if Input.down("f") then walk_x = walk_x - 1 end
    if Input.down("g") then walk_y = walk_y + 1 end
    if Input.down("t") then walk_y = walk_y - 1 end

    self.moving_x = walk_x
    self.moving_y = walk_y

    local running = (Input.down("r") or self.force_run) and not self.force_walk
...
function Player_3:updateWalk()
    if self:isMovementEnabled() then
        self:handleMovement()
    end
end

function Player_3:isMoving()
    return self.moving_x ~= 0 or self.moving_y ~= 0
end

function Player_3:beginSlide(last_state, in_place, lock_movement)
...
function Player_3:updateSlideDust()
...
function Player_3:endSlide(next_state)
