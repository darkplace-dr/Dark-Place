local Player_4, super = Class(Character)

function Player_4:init(chara, x, y, other)
    super.init(self, chara, x, y, other)

    self.is_player_4 = true
...
function Player_4:getDebugInfo()
...
function Player_4:getDebugOptions(context)
    context = super.getDebugOptions(self, context)
    context:addMenuItem("Toggle force run", "Toggle if the player_4 is forced to run or not", function() self.force_run = not self.force_run end)
    context:addMenuItem("Toggle force walk", "Toggle if the player_4 is forced to walk or not", function() self.force_walk = not self.force_walk end)
    return context
end

function Player_4:onAdd(parent)
    super.onAdd(self, parent)

    if parent:includes(World) and not parent.player_4 then
        parent.player_4 = self
    end
end

function Player_4:onRemove(parent)
    super.onRemove(self, parent)

    self.slide_sound:stop()
    if parent:includes(World) and parent.player_4 == self then
        parent.player_4 = nil
    end
end

function Player_4:onRemoveFromStage(stage)
    super.onRemoveFromStage(stage)
    self.slide_sound:stop()
end

function Player_4:setActor(actor)
...
function Player_4:setState(state, ...)
    self.state_manager:setState(state, ...)
end

function Player_4:isCameraAttachable()
    return not (self.state == "SLIDE" and self.slide_in_place)
end

function Player_4:isCamera2Attachable()
    return not (self.state == "SLIDE" and self.slide_in_place)
end

function Player_4:isMovementEnabled()
...
function Player_4:handleMovement()
    local walk_x = 0
    local walk_y = 0
...
function Player_4:updateWalk()
    if self:isMovementEnabled() then
        self:handleMovement()
    end
end

function Player_4:isMoving()
    return self.moving_x ~= 0 or self.moving_y ~= 0
end

function Player_4:beginSlide(last_state, in_place, lock_movement)
...
function Player_4:updateSlideDust()
...
function Player_4:endSlide(next_state)
