...
function Lib:init()
	Utils.hook(ActionBoxDisplay, 'draw', function(orig, self)
		...
		love.graphics.line(2, Game:getConfig("oldUIPositions") and 2 or 1, parent.realWidth,
			Game:getConfig("oldUIPositions") and 2 or 1)
		...
		love.graphics.rectangle("fill", 2, Game:getConfig("oldUIPositions") and 3 or 2, parent.realWidth - 2,
			Game:getConfig("oldUIPositions") and 34 or 35)
		...
		Utils.hook(AttackBox, 'update', function(orig, self, ...)
			...
			if not self.attacked then
				...
				while math.floor(self.afterimage_timer) > self.afterimage_count do
					...
					local afterimg = AttackBar(self.bolt_start_x - (self.afterimage_count * AttackBox.BOLTSPEED * 2), 0,
						6, self.realHeight)
					afterimg.layer = 3
					afterimg.alpha = 0.4
