...
function DarkPowerMenu:init()
    ...
    self.love = Game:getFlag("library_love")
    self.experience = Game:getFlag("library_experience")
    self.nextlv = Game:getFlag("library_nextlv")
...
function DarkPowerMenu:update()
    ...
    elseif self.state == "SPELLS" then
        ...
        if self.selected_spell ~= old_selected then
            local min_scroll = math.max(1, self.selected_spell - 3)
            local max_scroll = math.min(math.max(1, #spells - 3), self.selected_spell)
...
function DarkPowerMenu:drawSpells()
    ...
    for i = self.scroll_y, math.min(#spells, self.scroll_y + 3) do
    ...
    if self.state == "SPELLS" then
        ...
        if #spells > 4 then
            ...
            if self.scroll_y + 4 <= #spells then
                love.graphics.draw(self.arrow_sprite, 469, 214 + sine_off)
            end
            if self.scroll_y > 1 then
                love.graphics.draw(self.arrow_sprite, 469, 138 - sine_off, 0, 1, -1)
            end
            love.graphics.setColor(0.25, 0.25, 0.25)
            love.graphics.rectangle("fill", 473, 148, 6, 60)
            local percent = (self.scroll_y - 1) / (#spells - 4)
            love.graphics.setColor(1, 1, 1)
            love.graphics.rectangle("fill", 473, 148 + math.floor(percent * (60-6)), 6, 6)
        end
    end
end
...
function DarkPowerMenu:getExp()
    local party = self.party:getSelected()
    local exp = party:getExp()
    return exp
end

function DarkPowerMenu:getNextLv()
    local party = self.party:getSelected()
    local next = party:getNextLv()
    return next
end
...
function DarkPowerMenu:drawExp()
    ...
    if Kristal.getLibConfig("leveling", "global_love") then
        ...
        love.graphics.print(self.love, 290, 240)
        love.graphics.print("EXP:", 225, 265)
        love.graphics.print(self.experience, 275, 265)
        love.graphics.print("Next:", 350, 265)
        love.graphics.print(self.nextlv, 415, 265)
    else
        love.graphics.print("EXP:", 225, 265)
