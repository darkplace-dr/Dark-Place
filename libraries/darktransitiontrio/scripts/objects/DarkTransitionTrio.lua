local DarkTransitionTrio, super = Class(Object)

DarkTransitionTrio.SPRITE_DEPENDENCIES = {
...
function DarkTransitionTrio:init(final_y, options)
	...
	-- added
	self.third_x = options["third_x"] or 145
	self.third_y = options["third_y"] or 86
	--
	...
	-- added
    self.third_sprite_holder = Object(self.third_x, self.third_y)
	self.third_sprite_holder.layer = self.third_sprite_holder.layer - 1
    self.third_sprite = Sprite(nil, 0, 0)
    self.third_sprite_2 = Sprite(nil, 0, 0)
    self.third_sprite_3 = Sprite(nil, 0, 0)
	
    self.third_sprite.visible = false
    self.third_sprite_2.visible = false
    self.third_sprite_3.visible = false

    self.third_sprite_holder:addChild(self.third_sprite)
    self.third_sprite_holder:addChild(self.third_sprite_2)
    self.third_sprite_holder:addChild(self.third_sprite_3)
    self:addChild(self.third_sprite_holder)
	--
    ...
    self.spr_thirdu_run = Assets.getFrames(path .. "dark_transition/run")
    self.spr_third_fall_turnaround = Assets.getFrames(path .. "dark_transition/turn")
    self.spr_third_fall_d_lw = Assets.getFrames(path .. "dark_transition/light")
    self.spr_third_fall_d_dw = Assets.getFrames(path .. "dark_transition/dark")
    self.spr_third_fall_d_white = Assets.getFrames(path .. "dark_transition/white")
    self.spr_third_fall_smear = Assets.getFrames(path .. "dark_transition/smear")
...
function DarkTransitionTrio:onAddToStage(stage)
    for _,music in ipairs(Music.getPlaying()) do
        music:fade(0, 20/30)
    end
end

function DarkTransitionTrio:resumeTransition()
    self.waiting = false
end

function DarkTransitionTrio:drawDoor(x, y, xscale, yscale, rot, color)
...
function DarkTransitionTrio:update()
    ...
    if (self.velocity ~= 0) then
		...
		-- added
        self.third_y = self.third_y + self.velocity * DTMULT
		--
    end
...
function DarkTransitionTrio:draw()
    ...
    if (self.con == 8) then
		...
		-- added
        self.third_sprite.visible = true
        self.third_sprite:setFrames(self.spr_thirdu)
		--
    ...
    if (self.con == 9) then
        ...
        if (math.floor(self.timer) >= 68) and not self.do_once3 then
			...
			-- added
			self.third_sprite:setFrames(self.spr_thirdu_fall_lw)
			--
        end
    ...
    if (self.con == 17) then
		...
		-- added
        self.third_x_current = self.third_x
		--
		...
		-- added
		self.third_sprite:setFrames(self.spr_third_fall_turnaround)
		--
    end
    ...
    if (self.con == 18) then
        ...
        if (self.timer >= 35) then
			...
			-- added
			self.third_sprite:setFrames(self.spr_third_fall_d_lw)
			--
            -----------------
        end
    end
    ...
    if (self.con == 30) then
		...
		-- added
        self.third_sprite:setFrame(math.floor(self.index / 4) + 1)
		--
        ...
        if (self.timer >= 15) then
            if self.mod_loading then
				...
				-- added
                self.third_top = self.third_height

                self.third_sprite_2:setFrames(self.spr_third_fall_d_white)
                self.third_sprite_3:setFrames(self.spr_third_fall_d_dw)

                self.third_sprite.cutout_bottom = 0
                self.third_sprite_2.cutout_top = self.third_top
                self.third_sprite_3.cutout_top = self.third_top

                self.third_sprite_2.visible = true
                self.third_sprite_3.visible = true
				--
            end
        end
    end
    if (self.con == 31) then
		...
		-- added
        self.third_sprite:setFrame(math.floor(self.index / 4) + 1)
        self.third_sprite_2:setFrame(math.floor(self.index / 4) + 1)
        self.third_sprite_3:setFrame(math.floor(self.index / 4) + 1)
		
        if self.third_top == 0 then
            self.third_sprite_2.visible = false
        end
		--
		...
		-- added
        self.third_sprite.cutout_bottom = self.third_height - self.third_top
        self.third_sprite_2.cutout_top = self.third_top
        self.third_sprite_2.cutout_bottom = self.third_height - self.third_top - 1
        self.third_sprite_3.cutout_top = self.third_top + 1
		--
        ...
        if (self.timer >= 4) then
            ...
            -- added
			if (self.third_top > 5) then
                self.third_top = self.third_top - 0.5 * DTMULT
                if (self.quick_mode) then
                    self.third_top = self.third_top - 1.5 * DTMULT
                end
            else
                self.third_top = 0
            end
            if (self.third_top >= 2) then
                local x = ((self.third_x + 3) + math.random((self.third_width - 6)))
                local y = (self.third_y + self.third_top)
        ...
        if (self.timer >= self.threshold) then
			...
			-- added
            self.third_y = Utils.round(self.third_y)
            self.third_x = Utils.round(self.third_x)
			
            self.third_sprite:setFrames(self.spr_third_fall_smear)
            self.third_sprite:setFrame(1)

            self.third_sprite_2.visible = false
            self.third_sprite_3.visible = false

            self.third_sprite:setCutout()
            self.third_sprite_2:setCutout()
            self.third_sprite_3:setCutout()
			--
        end
    end
    if (self.con == 32) then
        ...
        if (self.timer >= 14) then
            ...
            -- added
			self.third_sprite:setFrames(self.spr_third_fall_ball)
            self.third_sprite_2:setFrames(self.spr_third_fall_ball)
            self.third_sprite_3:setFrames(self.spr_third_fall_ball)

            self.third_sprite_2.visible = true
            self.third_sprite_3.visible = true

            self.third_sprite_2.alpha = 0.5
            self.third_sprite_3.alpha = 0.25
			--
        end
    end
    if (self.con == 33) then
		...
		-- added
        self.third_sprite:setFrame(math.floor(self.timer / 2) + 1)
        self.third_sprite_2:setFrame(math.floor(self.timer / 2) + 1)
        self.third_sprite_3:setFrame(math.floor(self.timer / 2) + 1)
		
        self.third_sprite_2.y = -self.velocity
        self.third_sprite_3.y = -self.velocity * 2
		--
        -----------------
        ...
        if (self.timer > 30) then
            ...
            if (self.susie_y >= (self.final_y / 2) - self.susie_height) then
				...
				-- added
                self.third_sprite:setFrames(self.spr_third_dw_landed)
                ...
                self.third_y  = (self.final_y / 2) - self.third_sprite.height
				...
				-- added
                self.remthirdx = self.third_x
                self.remthirdy = self.third_y
				--
    ...
    if (self.con == 34) then
        ...
        if (self.timer > 1) then
			...
			-- added
            self.third_sprite:setFrame(math.floor(self.getup_index) + 1)
        ...
        if ((math.floor(self.timer) >= 26) and not self.do_once8) then
			...
			-- added
            -- self.third_x  = self.remthirdx
            -- self.third_y  = self.remthirdy
			--
            --scr_become_dark()
    ...
    if self.use_sprite_index then
		-- added
		self.third_sprite:setFrame(math.floor(self.sprite_index) + 1)
		--
	...
	-- added
    self.third_sprite_holder:setPosition(self.third_x + self.fake_shakeamount, self.third_y)
	--
