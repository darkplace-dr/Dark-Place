...
function Basic:init()
    ...
    self.arm_cannon_angle = math.rad(-170)
	
    self.angle_final = math.rad(-170)
...
function Basic:draw()
    ...
    if self.variant == 3 and self.movecon == 1 then
        if self.con == 0 then
            ...
            if self.attack_timer >= 1 then
                local wall = self:addChild(PipisWall(Game.battle.arena.x + 200, Game.battle.arena.bottom))
                wall.type = 1
                self.con = 1
                self.attack_timer = 0
            end
        ...
        if self.con == 1 then
            for i = 1, 3 do
                if (Utils.random(10) < 6) then
                    bul = self:spawnBullet("sneo/pipis", (xx + 5) + (math.sin(self.arm_cannon_angle) * 50), yy + (math.cos(self.arm_cannon_angle + math.rad(270)) * 50))
                    bul.type = 2
                    ...
                    bul = self:spawnBullet("sneo/sneobomb_pipis", (xx + 5) + (math.sin(self.arm_cannon_angle) * 50), yy + (math.cos(self.arm_cannon_angle + math.rad(270)) * 50))
                    bul.physics.gravity = (0.2 + (Utils.random(3) / 100))
                    bul.physics.speed_y = (-6 - Utils.random(2))
           ...
           --[[for i = 0, 3 do
                bul = self:spawnBullet(obj_sneo_armcannon_smoke, ((xx + 5) + lengthdir_x(50, (image_angle + 270))), (yy + lengthdir_y(50, (image_angle + 270))))
    ...
    if self.variant == 2 or self.variant == 3 then
        Draw.draw(Assets.getTexture("npcs/sneo/wing_l"), (xx - 20), (yy - 40), math.rad(20), 2, 2)
        Draw.draw(self.arm_cannon_spr, xx, ((yy + 11) + (shake / 2)), math.rad(self.arm_cannon_angle + shake), self.arm_cannon_scale_x, 2)
        Draw.draw(Assets.getTexture("npcs/sneo/body"), (xx - 18), (yy - 40), 0, 2, 2)
        Draw.draw(Assets.getTexture("npcs/sneo/head_2"), (xx - 30), (yy - 29), 0, 2, 2)
    end
end
