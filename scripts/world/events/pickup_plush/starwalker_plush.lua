---@class starwalker_plush : Pickup
---@overload fun(...) : starwalker_plush
local starwalker_plush, super = Class(Event, "starwalker_plush")

function starwalker_plush:init(data)
    ...
    self.sprite = Sprite("world/events/pickup_plush/starwalker_plush")
...
function starwalker_plush:postLoad()
	self.old_parent = self.parent
end

function starwalker_plush:onInteract(player, dir)
...
function starwalker_plush:update()
	super.update(self)
...
function starwalker_plush:canPlace(player)
	return not Game.world:checkCollision(player.interact_collider[player.facing])
end

function starwalker_plush:onRemove(parent)
	self.data = nil
