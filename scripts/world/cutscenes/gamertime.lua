return {
    ...
    berdly = function(cutscene, event)
        ...
        if berdly then
            ...
            cutscene:walkTo(brenda, x + 30, y + 60, 1, "up")
            cutscene:wait(1)
            cutscene:detachCamera()
            cutscene:showNametag("Susie")
            cutscene:text("* A door,[wait:5] huh...?", "neutral_side", "susie")
            cutscene:text("* It looks different from all the others.", "neutral", "susie")
            cutscene:text("* And it looks tough.[wait:5] Might be hard to open it.", "nervous_side", "susie")
            cutscene:showNametag("Brenda")
            cutscene:text("* But that's only if it's locked.[wait:5] We haven't tried to open it yet.", "neutral_side", "brenda")
            cutscene:showNametag("Susie")
            cutscene:text("* Good point.", "nervous", "susie")
            cutscene:hideNametag()
            cutscene:wait(cutscene:walkTo(susie, x, y + 20, 0.5, "up", true))
            cutscene:wait(2)
            susie:setFacing("down")
            cutscene:showNametag("Susie")
            cutscene:text("* Uh,[wait:5] no.[wait:5] We totally need something like a bomb to open it.", "annoyed", "susie")
            cutscene:showNametag("Brenda")
            if Game.inventory:hasItem("bomb") then
                cutscene:showNametag("Brenda")
                cutscene:text("* Good thing we already have it.", "happy_side", "brenda")
                cutscene:showNametag("Susie")
                cutscene:text("* Totally.", "closed_grin", "susie")
            else
                cutscene:text("* Well,[wait:5] then what are we waiting for?", "happy", "brenda")
                cutscene:text("* Let's go find it,[wait:5] shall we?", "happy_b", "brenda")
            end
            cutscene:hideNametag()
            cutscene:attachFollowers()
