...
function spell:getDamage(user, target)
    if Game:isLight() then
        local min_magic = MathUtils.clamp(user.chara:getStat("magic") - 3, 1, 999)
        return math.ceil((min_magic * 8) + 45 + MathUtils.random(10))
    else
		local min_magic = MathUtils.clamp(user.chara:getStat("magic") - 10, 1, 999)
		return math.ceil((min_magic * 12) + 90 + MathUtils.random(10))
    end
end
