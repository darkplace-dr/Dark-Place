...
function item:onWorldUse(target)
    ...
    Game.world:startCutscene(function(cutscene)
        cutscene:text("* (You inject "..(is_leader and "yourself" or target.name).." with the content of the syringe...)")
            ...
            target.poison_effect_timer = Game.world.timer:every(1, function()
                if not Game.world.cutscene then -- Don't hurt anyone during a cutscene
                    if poison_left == 0 then
                        Game.world:showText("* ("..(is_leader and "You feel" or target.name.." feels").." better now.)")
                        return false
                    end
                    if target.health > 1 then
                        ...
                        poison_left = 0
                        Game.world:showText("* ("..(is_leader and "You feel" or target.name.." feels").." dead inside now.)")
                        return false
                    end
                end
            end)
        elseif effect >= 10 and effect < 30 then
            cutscene:text("* (Looks like "..(is_leader and "your" or target.name.."'s")..(target.ribbit and " placebos" or " defense").." decreased...\nTemporarely.)")
        ...
        elseif effect == 50 then
            cutscene:text("* ("..(is_leader and "You don't" or target.name.." doesn't").." feel better, nor worse.)")
        elseif effect > 50 and effect < 70 then
            target:heal(1)
            cutscene:text("* (It healed... 1HP?)")
        elseif effect >= 70 and effect < 90 then
            cutscene:text("* (Looks like "..(is_leader and "your" or target.name.."'s")..(target.ribbit and " placebos" or " defense").." increased...\nTemporarely.)")
            target:increaseStat("defense", 10)
            target.defense_effect_timer = Game.world.timer:after(300, function()
                Assets.playSound("boost")
                target:increaseStat("defense", -10)
                if not Game.world.cutscene then
                    Game.world:showText("* ("..(is_leader and "You feel" or target.name.." feels").." normal again.)")
                end
            end)
