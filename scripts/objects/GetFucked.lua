local GameOver, super = Class(Object, "GetFucked")
    ...
    if (self.timer >= 150) and (self.current_stage == 4) then
        ...
        if not Game:getConfig("oldGameOver") or Game:isLight() then
            if Game:isLight() then
                ...
                self.text = Sprite("ui/getfucked", 44, 40)
                self.text:setScale(3)
            end
            self.alpha = 0
    ...
    if (self.current_stage == 8) then
        if self.choicer.choice then
            ...
            if self.choicer.choice_x == 1 then
                self.text:setSprite("ui/getdoublefucked")
                self.text:setPosition(12, self.text.y)
                self.text:setScale(2.5)
                if self.explosion == nil then
                    self.explosion = Explosion(160+36+20, 360)
                    self:addChild(self.explosion)
                end
                self.timer = 0
                Game.stage.timer:after(2.75, function()
                    if self.achievementupped == nil then
                        self.achievementupped = true
                        if AchLib:getAchievement("explosion").earned ~= true then
                            AchLib:addAchProgress("explosion", 1)
                        end
                    end
