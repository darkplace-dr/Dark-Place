...
function PlayerBoxDisplay:draw()
    ...
    local health = (Game.card_game.player_hp / Game.card_game.max_hp) * 76

    if health > 0 then
        love.graphics.setColor(Game.party[1].color)
    ...
    elseif (Game.card_game.player_hp <= (Game.card_game.max_hp / 4)) then
    ...
    health_offset = (#tostring(Game.card_game.player_hp) - 1) * 8

    love.graphics.setFont(self.font)
    love.graphics.print(Game.card_game.player_hp, 152 - health_offset, 9 - self.actbox.data_offset)
	...
	love.graphics.setColor({0,0,0,1})
	love.graphics.rectangle("fill", 0, 37, 50, 37)
